...
class Review extends React.Component {
  ...
  componentWillMount() {
    if (this.state && this.state.user) this.setCet4Data();
  }
  callback = key => {
    if (key === "1") {
      this.setCet4Data();
    ...
    axios({
      ...
      .then(response => {
        ...
        this.setState({
          ...
          pre_no: 0,
          test_count: response.data.data.words_list.length
        });
      })
              ...
              <TabPane className="recite_tabpane" tab="四级" key="1">
                <div className="oneword" style={{ margin: "5rem 2rem" }}>
                  <OneWord
                    ...
                    word={
                      this.state.words_list
                        ? this.state.words_list[this.state.pre_no].word
                        : ""
                    }
                    desc={
                      this.state.words_list
                        ? this.state.words_list[this.state.pre_no].desc
                        : ""
                    }
